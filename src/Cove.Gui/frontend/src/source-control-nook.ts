...
export async function renderSourceControlNook(repoRoot: string, openFile?: (path: string) => void): Promise<HTMLElement> {
  const el = document.createElement("div");
  el.className = "source-control-nook";
  el.tabIndex = 0;
  ...
  const refresh = async () => {
    ...
    try {
      ...
      rows = [];
      fileList.innerHTML = `<div style="padding:20px;color:var(--muted);font-size:12px;">${repoRoot ? `Not a usable git repository: ${repoRoot}` : "This bay has no directory"}</div>`;
    }
  };
