...
public static class CollectionCommands
{
    ...
    [CoveCommand("cove://commands/collection.move-workspace")]
    public static async Task<ControlResponse> CollectionMoveWorkspace(EngineDispatchContext ctx)
    {
        if (ctx.Workspaces is not { } manager)
            return ctx.Fail("no_workspaces", "workspace manager unavailable");
        if (ctx.Request.Params is not JsonElement el
            || el.Deserialize(CollectionJsonContext.Default.CollectionMoveParams) is not { } p)
            return ctx.Fail("bad_params", "workspaceId and collectionId are required");
        return await manager.MoveWorkspaceToCollectionAsync(p.WorkspaceId, p.CollectionId).ConfigureAwait(false)
            ? ctx.Ok()
            : ctx.Fail("not_found", "workspace or collection not found");
    }
...
public sealed record CollectionMoveParams(string WorkspaceId, string CollectionId);
