...
public static class StatusCommands
{
    ...
    public static Task<ControlResponse> List(EngineDispatchContext ctx)
    {
        ...
        var statuses = svc.ListStatuses(p.WorkspaceId, includeHidden: true).Select(ToInfo).ToList();
    ...
    public static async Task<ControlResponse> Create(EngineDispatchContext ctx)
    {
        ...
        var row = await svc.CreateStatusAsync(p.WorkspaceId, p.Id, p.Name, p.HexColor, p.Position);
        if (row is null)
    ...
    public static async Task<ControlResponse> Delete(EngineDispatchContext ctx)
    {
        ...
        try
        {
            await svc.DeleteStatusAsync(p.WorkspaceId, p.Id, p.RehomeToStatusId);
            return ctx.Ok();
        }
    ...
    public static async Task<ControlResponse> Reorder(EngineDispatchContext ctx)
    {
        ...
        await svc.ReorderStatusesAsync(p.WorkspaceId, p.OrderedIds);
        return ctx.Ok();
    }
    ...
    public static async Task<ControlResponse> SetHidden(EngineDispatchContext ctx)
    {
        ...
        await svc.SetStatusHiddenAsync(p.WorkspaceId, p.Id, p.Hidden);
        return ctx.Ok();
    }

    private static StatusInfo ToInfo(Cove.Tasks.Store.StatusRow row) => new(
        row.WorkspaceId, row.Id, row.Name, row.HexColor, row.Position, row.Hidden, row.IsLooping, row.IsInProgress, row.IsReview, row.IsCompletion);
}
