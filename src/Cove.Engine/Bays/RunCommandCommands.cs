...
{
    [CoveCommand("cove://commands/bay-command.create")]
    public static async Task<ControlResponse> Create(EngineDispatchContext ctx)
    {
        ...
        if (ctx.Request.Params is not JsonElement el
            ...
            || string.IsNullOrWhiteSpace(p.BayId) || string.IsNullOrWhiteSpace(p.Label) || string.IsNullOrWhiteSpace(p.Command))
            return ctx.Fail("bad_params", "bayId, label and command are required");

        var def = await svc.CreateAsync(p.BayId, p.Label, p.Command, p.Cwd).ConfigureAwait(false);
        return ctx.Ok(new RunCommandDefResult(def.Id, def.BayId, def.Label, def.Command, def.Cwd), RunCommandVerbJsonContext.Default.RunCommandDefResult);
    }

    [CoveCommand("cove://commands/bay-command.edit")]
    public static async Task<ControlResponse> Edit(EngineDispatchContext ctx)
    {
        ...
        return edited is null
            ...
            : ctx.Ok(new RunCommandDefResult(edited.Id, edited.BayId, edited.Label, edited.Command, edited.Cwd), RunCommandVerbJsonContext.Default.RunCommandDefResult);
    }

    [CoveCommand("cove://commands/bay-command.delete")]
    ...
    [CoveCommand("cove://commands/bay-command.list")]
    public static async Task<ControlResponse> List(EngineDispatchContext ctx)
    {
        ...
        if (ctx.Request.Params is not JsonElement el
            ...
            || string.IsNullOrWhiteSpace(p.BayId))
            return ctx.Fail("bad_params", "bayId is required");

        var items = await svc.ListEffectiveAsync(p.BayId, p.ParentBayId).ConfigureAwait(false);
        var result = items.Select(i => new RunCommandListItemDto(
            i.Definition.Id, i.Definition.BayId, i.Definition.Label, i.Definition.Command, i.Definition.Cwd,
    ...
    [CoveCommand("cove://commands/bay-command.status")]
    ...
    [CoveCommand("cove://commands/bay-command.start")]
    ...
    [CoveCommand("cove://commands/bay-command.stop")]
    ...
    [CoveCommand("cove://commands/bay-command.restart")]
    ...
    [CoveCommand("cove://commands/bay-command.logs")]
    ...
    [CoveCommand("cove://commands/bay-command.clear")]
...
public sealed record RunCommandCreateParams(string BayId, string Label, string Command, string? Cwd = null);
public sealed record RunCommandEditParams(string Id, string Label, string Command, string? Cwd = null);
public sealed record RunCommandListParams(string BayId, string? ParentBayId = null);
public sealed record RunCommandLogsParams(string Id, int? Tail = null);
public sealed record RunCommandIdParams(string Id);
public sealed record RunCommandDefResult(string Id, string BayId, string Label, string Command, string Cwd);
public sealed record RunCommandListItemDto(string Id, string BayId, string Label, string Command, string Cwd, string Lifecycle, bool Inherited);
