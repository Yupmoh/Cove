...
internal static class McpBridgeCommands
{
    ...
    {
        return Task.FromResult(ctx.Ok(new IpcEventLog(System.Array.Empty<IpcEventEntry>()), CoveJsonContext.Default.IpcEventLog));
    }
    ...
    public static Task<ControlResponse> CaptureNativeScreenshot(EngineDispatchContext ctx)
        => Task.FromResult(ctx.Fail("no_render_client", "render-bound verb requires a GUI client"));
