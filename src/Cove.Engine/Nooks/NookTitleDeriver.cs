...
public sealed class NookTitleDeriver
{
    private readonly ILogger _logger;

    public NookTitleDeriver(ILogger? logger = null)
    {
    ...
    public string DeriveTitle(string? adapter, string? command, string? workingDirectory, string? customTitle)
    {
        ...
        _logger.LogWarning("nook-title: could not derive title from adapter={adapter} command={command} cwd={cwd}", adapter, command, workingDirectory);
        return "terminal";
    }
