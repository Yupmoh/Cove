...
# Resolve schema location: env var > script-relative > ~/.cove/sdk/schemas > ~/.atrium/sdk/schemas
if [[ -n "${COVE_SDK_SCHEMAS:-${atrium_SDK_SCHEMAS:-}}" ]] && [[ -d "${COVE_SDK_SCHEMAS:-${atrium_SDK_SCHEMAS}}" ]]; then
  SCHEMAS_DIR="${COVE_SDK_SCHEMAS:-${atrium_SDK_SCHEMAS}}"
else
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  if [[ -d "$SCRIPT_DIR/schemas" ]]; then
    SCHEMAS_DIR="$SCRIPT_DIR/schemas"
  elif [[ -d "$HOME/.cove/sdk/schemas" ]]; then
    SCHEMAS_DIR="$HOME/.cove/sdk/schemas"
  elif [[ -d "$HOME/.atrium/sdk/schemas" ]]; then
    SCHEMAS_DIR="$HOME/.atrium/sdk/schemas"
  else
    echo "${CROSS} Cannot find SDK schemas directory." >&2
    echo "  Set COVE_SDK_SCHEMAS or ensure ~/.cove/sdk/schemas/ exists." >&2
    exit 1
  fi
fi
