...
public sealed class LibraryStoreTests
{
    ...
    public void SaveEntry_ThenList_ReturnsEntry()
    {
        ...
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "My Session", """{"scroll":0}""", "some output", "history");

        var list = store.ListByBay("ws1");
        Assert.Single(list);
        Assert.Equal(entry.Id, list[0].Id);
        Assert.Equal("terminal", list[0].NookType);
    }
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", """{"password":"secret123"}""", null, "history");
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, "Authorization: Bearer my-secret-token", "history");
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", """{"api_key":"sk-abc123xyz"}""", null, "history");
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", """{"name":"test","count":42}""", "normal output", "history");
    ...
    public void SeededSecretNotPersisted()
    {
        ...
        store.SaveEntry("ws1", "nook-1", "terminal", "Test", $@"{{""password"":""{secretValue}""}}", $"Enter password: {secretValue}", "history");
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, null, "saved");
        store.Pin(entry.Id);
        var list = store.ListByBay("ws1");
        Assert.Equal(entry.Id, list[0].Id);
    }
    ...
    {
        var (_, store) = NewStore();
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, null, "history");
    ...
    {
        var (_, store) = NewStore();
        store.SaveEntry("ws1", "nook-1", "terminal", "A", null, null, "saved");
        store.SaveEntry("ws1", "nook-2", "terminal", "B", null, null, "history");

        var saved = store.ListByBay("ws1", "saved");
        var history = store.ListByBay("ws1", "history");
        Assert.Single(saved);
    ...
    public void SaveEntry_RedactsKeywordFreeAwsKey()
    {
        ...
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, $"export AWS_ACCESS_KEY_ID={awsKey}", "history");
    ...
    public void SaveEntry_RedactsKeywordFreeGitHubToken()
    {
        ...
        var entry = store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, $"echo {ghToken}", "history");
    ...
    public void SeededKeywordFreeSecretNotPersisted()
    {
        var (dir, store) = NewStore();
        var awsKey = "AKIAIOSFODNN7EXAMPLE";
        store.SaveEntry("ws1", "nook-1", "terminal", "Test", null, $"AWS_ACCESS_KEY_ID={awsKey}", "history");
