...
public sealed class NookTitleDeriverTests
{
    [Fact]
    public void DeriveTitle_CustomTitle_TakesPrecedence()
    {
        var deriver = new NookTitleDeriver(NullLogger.Instance);
        var title = deriver.DeriveTitle("claude", "/bin/bash", "/home/user", "My Nook");
        Assert.Equal("My Nook", title);
    }
