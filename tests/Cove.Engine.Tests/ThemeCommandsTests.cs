...
public sealed class ThemeCommandsTests
{
    ...
    public async Task ThemeSetActive_ReturnsThemeAndPersists()
    {
        ...
        {
            var themes = new ThemeService(dir);
            themes.SaveCustom(new Theme("my-theme", "dark", "#000000", "#ffffff", "#111111", "#eeeeee", "#ff0000"));
            var prm = JsonDocument.Parse("""{"name":"my-theme"}""").RootElement.Clone();
            var resp = await EngineCommandRouter.RouteAsync(new ControlRequest("r1", "cove://commands/theme.set-active", prm), themes: themes);
            Assert.True(resp!.Ok);
            Assert.Equal("my-theme", resp.Data!.Value.GetProperty("theme").GetProperty("name").GetString());
            ...
            Assert.Equal("my-theme", getResp.Data!.Value.GetProperty("theme").GetProperty("name").GetString());
        }
        finally { Cove.Testing.TestDirectory.Delete(dir); }
    }
    ...
    public async Task ThemeDeleteCustom_RejectsBuiltin()
    {
        ...
        {
            var themes = new ThemeService(dir);
            var prm = JsonDocument.Parse("""{"name":"catppuccin-mocha"}""").RootElement.Clone();
            var resp = await EngineCommandRouter.RouteAsync(new ControlRequest("r1", "cove://commands/theme.delete-custom", prm), themes: themes);
