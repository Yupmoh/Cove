...
public sealed class CanvasFormBindingTests
{
    ...
    public void FormBindingState_RoundTripsThroughStateJson()
    {
        ...
        var note = store.Create(new Note { Title = "Canvas Form", WorkspaceId = "ws1", Content = "{}", Source = "gui", Kind = "canvas" });
    ...
    public void CanvasNote_PersistsAndReloadsWithState()
    {
        ...
        var note = store.Create(new Note { Title = "Form Canvas", WorkspaceId = "ws1", Content = canvasJson, Source = "gui", Kind = "canvas" });
    ...
    {
        var (_, store) = NewStore();
        var note = store.Create(new Note { Title = "Stateful", WorkspaceId = "ws1", Content = "{}", Source = "gui", Kind = "canvas" });
