...
# test-adapter.sh — Programmatic end-to-end testing for cove adapters.
#
# What it does:
...
#      (if present) and diffs the result against expected-cove.json.
#   4. Runs the actual installed hook command end-to-end with synthetic
#      stdin, asserting exit code and expected stdout.
#   5. POSTs the normalized payload to cover-dev's /resolve endpoint
#      and asserts {"ok": true}.
#
# Channel: dev-only. Reads ~/.cover-dev/hook-port for the resolver
# port and ~/.cover-dev/bin/cover-dev for the CLI binary. Stable
# (~/.cove) is never touched.
#
# Usage:
...
#
# Env overrides:
#   COVER_TEST_PORT      Override hook-port discovery
#   COVER_TEST_NO_HTTP   If "1", skip the /resolve acceptance check
#   COVER_TEST_NO_INSTALL If "1", skip install/uninstall (test fixtures only)
...
COVER_DEV_DIR="${HOME}/.cover-dev"
COVER_PORT_FILE="${COVER_DEV_DIR}/hook-port"

if [[ -n "${COVER_TEST_PORT:-}" ]]; then
  RESOLVE_PORT="$COVER_TEST_PORT"
elif [[ -f "$COVER_PORT_FILE" ]]; then
  RESOLVE_PORT="$(cat "$COVER_PORT_FILE")"
else
  RESOLVE_PORT=""
fi
...
BACKUP_DIR="$(mktemp -d "/tmp/cove-test-${ADAPTER_NAME}.XXXXXX")"
...
backup_files() {
  for path in "${BACKUP_PATHS[@]}"; do
    ...
    if [[ -e "$expanded" ]]; then
      ...
      touch "$BACKUP_DIR$expanded.__cove_test_didnotexist"
    fi
  done
}
...
restore_backups() {
  for path in "${BACKUP_PATHS[@]}"; do
    expanded="${path/#\~/$HOME}"
    if [[ -f "$BACKUP_DIR$expanded.__cove_test_didnotexist" ]]; then
      rm -f "$expanded"
...
if [[ "${COVER_TEST_NO_INSTALL:-0}" = "1" ]]; then
  skip "install/uninstall (COVER_TEST_NO_INSTALL=1)"
...
if [[ ! -d "$FIXTURES_DIR" ]]; then
  ...
  for event_dir in "$FIXTURES_DIR"/*/; do
    ...
    expected_file="$event_dir/expected-cove.json"
    ...
    if [[ ! -f "$expected_file" ]]; then
      skip "$event: no expected-cove.json"
      continue
    fi
...
if [[ "${COVER_TEST_NO_HTTP:-0}" = "1" ]]; then
  skip "HTTP /resolve (COVER_TEST_NO_HTTP=1)"
elif [[ -z "$RESOLVE_PORT" ]]; then
  skip "no cover-dev hook-port file (~/.cover-dev/hook-port)"
...
section "Phase 5: cove captured state"

if [[ "${COVER_TEST_NO_HTTP:-0}" = "1" ]]; then
  skip "cove state (COVER_TEST_NO_HTTP=1)"
elif [[ -z "$RESOLVE_PORT" ]]; then
  skip "no cover-dev hook-port file"
else
  ...
  if ! echo "$state_response" | jq -e '.events | type == "array"' >/dev/null 2>&1; then
    ...
    for event_dir in "$FIXTURES_DIR"/*/; do
      [[ -d "$event_dir" ]] || continue
      [[ -f "$event_dir/expected-cove.json" ]] && expected_count=$((expected_count + 1))
    done

    if [[ "$captured_count" -eq "$expected_count" ]]; then
      pass "cove captured $captured_count event(s) (matches $expected_count fixture(s))"
    else
      fail "cove captured $captured_count event(s) but $expected_count fixture(s) were posted" \
    ...
    # should round-trip through cove intact.
    for event_dir in "$FIXTURES_DIR"/*/; do
      [[ -d "$event_dir" ]] || continue
      expected_file="$event_dir/expected-cove.json"
      [[ -f "$expected_file" ]] || continue
