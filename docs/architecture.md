...
Cove is headless-first. A single detached engine process (`cove daemon run`) owns the PTYs, sessions, bays, and persistence. The desktop GUI (a Ryn native-webview app), the terminal-mode client, and the `cove` CLI are all clients of that engine over a local control socket. This split is a day-one invariant, not a retrofit.
...
| Frontend (Vite + TypeScript + xterm.js) | GUI nooks in the Ryn webview | Perf proof in M0; built in M1 |
